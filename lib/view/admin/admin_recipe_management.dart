...
class _AdminRecipeManagementState extends State<AdminRecipeManagement> {
              ...
              Expanded(
                child: Container(
                  ...
                  child: DropdownButton<String>(
                    ...
                    items: const [
                      DropdownMenuItem(
                          value: 'all', child: Text('All Recipes')),
                      DropdownMenuItem(
                          value: 'public', child: Text('Public Only')),
                      DropdownMenuItem(
                          value: 'private', child: Text('Private Only')),
                    ],
                    onChanged: (value) {
                      setState(() {
        ...
        if (snapshot.hasData) {
          ...
          return Container(
            ...
            child: Row(
              ...
              children: [
                _buildStatCard('Total', stats['totalRecipes'].toString(),
                    Colors.blue, Icons.restaurant),
                _buildStatCard('Public', stats['publicRecipes'].toString(),
                    Colors.green, Icons.public),
                _buildStatCard('Private', stats['privateRecipes'].toString(),
                    Colors.orange, Icons.lock),
                if (_isSelectionMode)
                  _buildStatCard(
                      'Selected',
                      _selectedRecipeIds.length.toString(),
                      Colors.purple,
                      Icons.check_circle),
              ],
            ),
                    ...
                    : PopupMenuButton<String>(
                        ...
                        itemBuilder: (context) => [
                          ...
                          PopupMenuItem(
                            ...
                            child: Row(
                              children: [
                                Icon(recipe.isPublic
                                    ? Icons.lock
                                    : Icons.public),
                                const SizedBox(width: 8),
                                Text(recipe.isPublic
                                    ? 'Make Private'
                                    : 'Make Public'),
                              ],
                            ),
  ...
  void _handleRecipeAction(String action, RecipeModel recipe) {
    switch (action) {
      case 'view_details':
        _showRecipeDetails(recipe);
        break;
  ...
  void _toggleRecipeVisibility(RecipeModel recipe) async {
    setState(() => _isLoading = true);

    bool success = await _recipeService.toggleRecipeVisibility(
  ...
  void _bulkDeleteRecipes() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to delete ${_selectedRecipeIds.length} selected recipes? This action cannot be undone.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Cancel'),
  ...
  void _performBulkDelete() async {
    ...
    int deletedCount =
        await _recipeService.bulkDeleteRecipes(_selectedRecipeIds);
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoCard([
                        ...
                        _buildInfoRow(
                            Icons.star_rate, 'Rating', '${recipe.rating}/5.0'),
                        _buildInfoRow(
                          recipe.isPublic ? Icons.public : Icons.lock,
                          'Visibility',
                          recipe.isPublic ? 'Public' : 'Private',
                          valueColor:
                              recipe.isPublic ? Colors.green : Colors.orange,
                        ),
                      ]),
                      ...
                      if (recipe.ingredients.isNotEmpty) ...[
                        ...
                        _buildInfoCard(
                          recipe.ingredients
                              .asMap()
                              .entries
                              .map((entry) => _buildIngredientRow(entry.key + 1,
                                  _formatIngredient(entry.value)))
                              .toList(),
                        ),
                      ],
                      ...
                      if (recipe.instructions.isNotEmpty) ...[
                        ...
                        _buildInfoCard(
                          recipe.instructions
                              .asMap()
                              .entries
                              .map((entry) => _buildInstructionRow(
                                  entry.key + 1,
                                  _formatInstruction(entry.value)))
                              .toList(),
                        ),
                      ],
                    ],
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ElevatedButton.icon(
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor:
                              recipe.isPublic ? Colors.orange : Colors.green,
                          foregroundColor: Colors.white,
                          padding: const EdgeInsets.symmetric(vertical: 12),
                        ),
              ...
              Expanded(
                child: StreamBuilder<List<RecipeModel>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    List<RecipeModel> userRecipes = snapshot.data!;
                              ...
                              return Card(
                                ...
                                child: ListTile(
                                  ...
                                  trailing: PopupMenuButton<String>(
                                    onSelected: (value) {
                                      Navigator.pop(
                                          context); // Close this dialog first
                                      _handleRecipeAction(value, recipe);
                                    },
                                    itemBuilder: (context) => [
                                      const PopupMenuItem(
  ...
  String _formatIngredient(String ingredient) {
    ...
    if (ingredient.contains('name:') &&
        ingredient.contains('unit:') &&
        ingredient.contains('amount:')) {
      try {
